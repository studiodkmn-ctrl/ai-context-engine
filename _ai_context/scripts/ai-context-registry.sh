...
project_dir = ctx_dir.parent

# v7 Frische-Modell: alter Scan-Stand als Fallback fuer 'seen' bei Chunks
# ohne explizite seen:-Zeile im Text (siehe _gotchas.md#legende).
old_registry_by_id = {
...
for rel_file, chunk_type in KNOWLEDGE_FILES:
    ...
    for m in found:
        ...
        # v7 Frische-Modell: seen (wann bestaetigt) vs. code_touched (git-Historie
        # der @-Dateien) -> status. seen-Herkunft: explizite seen:-Zeile > alter
        # Registry-Scan (updated/seen) > heute (Erst-Erfassung).
        old_entry = old_registry_by_id.get(cid, {})
        seen = (
            ctxlib.extract_seen_field(text)
            or old_entry.get('seen')
            or old_entry.get('updated')
            or date_short
        )
        at_files = ctxlib.parse_at_files(text)
        code_touched = ctxlib.git_last_touched(at_files, cwd=str(project_dir)) if at_files else None
        if at_files and not any((project_dir / f).exists() for f in at_files):
            status = 'orphan'
        elif code_touched and code_touched > seen:
            status = 'check'
        else:
            status = 'fresh'
        ...
        chunks.append({
            ...
            'seen': seen, 'code_touched': code_touched or '', 'status': status,
        })

# registry.yaml schreiben
lines = [
    "# AI Context Registry v7.0",
    ...
    "# updated:      letzter Scan dieses Chunks (Registry-Zeitstempel)",
    "# seen:         wann der Eintrag zuletzt inhaltlich bestaetigt wurde",
    "# code_touched: juengste git-Aenderung der @-Dateien (leer = unbekannt/keine Datei)",
    "# status:       fresh (seen aktuell) | check (Code neuer als seen) | orphan (@-Dateien fehlen)",
    f'version: "7.0"',
