#!/usr/bin/env bash
# =============================================================================
# install.sh — Globale Installation von AI Context v6.5
#
# Modi:
...
if $PRO_MODE; then
  echo -e "${BOLD}🧠 AI Context v6.5 — Pro Installation${NC}"
  echo -e "   Basis + Ollama + RAG-Cache + Embeddings"
else
  echo -e "${BOLD}🧠 AI Context v6.5 — Simple Installation${NC}"
...
# AI Context v6.5 — Ollama-Konfiguration
...
# =============================================================================
echo ""
echo -e "${GREEN}✅ AI Context v6.5 installiert${NC}"
echo ""
...
if $PRO_MODE; then
  ...
  echo -e "  ${CYAN}ai-context-sync --export${NC}    → Für Claude Chat (Clipboard)"
  echo ""
  echo -e "${BOLD}Claude Code Skills:${NC}  ${CYAN}/ai-doctor${NC} (check only)"
  echo ""
  echo -e "${BOLD}Pro-Features freischalten:${NC}"
  echo -e "  ${CYAN}bash install.sh --pro${NC}"
  echo -e "  → Ollama • /ai-fix • --fix • /ai-transfer • RAG-Cache • Impact-Graph"
fi
echo ""
